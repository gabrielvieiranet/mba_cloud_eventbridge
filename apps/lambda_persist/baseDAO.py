...
class BaseDAO:
    ...
    def query_table_key_between_range_key(
            self,
            partition_key,
            partition_key_value,
            range_key,
        ...
        filtering_exp = Key(partition_key).eq(partition_key_value) & Key(
            range_key).between(range_key_value1, range_key_value2)
        response = self.__table.query(
            KeyConditionExpression=filtering_exp
        )
        return response

    def query_table_key_between_range_key_on_secondaryIndex(
            self,
            partition_key,
            partition_key_value,
            range_key,
    ...
    def query_table_key_and_range_key(
            self,
            partition_key,
            partition_key_value,
            range_key,
        ...
        filtering_exp = Key(partition_key).eq(
            partition_key_value) & Key(range_key).eq(range_key_value)
        response = self.__table.query(
            KeyConditionExpression=filtering_exp
        )
        return response

    def query_table_key_and_range_key_on_secondaryIndex(
            self,
            partition_key,
            partition_key_value,
            range_key,
